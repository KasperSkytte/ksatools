...
#'
#' @param install Install the package (\code{TRUE}) with \code{\link[devtools]{install}} or just load it in memory (\code{FALSE}) with \code{\link[devtools]{load_all}}. (Default: \code{TRUE})
...
ksa_gogoawesomepkg <- function(install = TRUE,
                               pkgdown = TRUE,
                               verbose = TRUE) {
  outlist <- list()
